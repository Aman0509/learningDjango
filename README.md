...
Until now, we were storing our data either in file or assigning to some variables. However, data's persistent life is only till our servers are up and if servers are restarted, then all data would be vanished. Hence, we need database to store our data and is a more permanent solution.
...
Above are some examples SQL queries which shows the creation of table and inserting data into it. However, with Django, we can altogether avoid writing queries by using Models.
...
- Migrations needs to be executed every time whenever model is updated.

We will use the Model shown in previous example for creating table in database based on it. For that, we will first run command:
...
We will first use Django shell to perform data insert option and afterwards, we will see how same can be implemented with **views**.
...
Interactive shell will be open
...
- To see the data saved by us, we can query database. For that, we have to use a **Model** class static attribute **object**.

- Now, with the use of **object** attribute with our own Model class, we get an object and on that, several methods can be used.
...
1. Define __str__ to print more user friendly instance name instead of default.
2. Add more fields.
...
- Let's see how to use slug here.
  ...
  - But how can be automatically update the slug field for each newly added book?
...
Database indexes are a technical detail of databases, SQL and NoSQL databases. It basically means that the database will save the values in that column a bit more efficiently or in a way that makes searching them a bit more efficient and that's some behind the scenes stuff which you don't need to know.
...
### [book_store Project](03-Data_and_Models/book_store/)
